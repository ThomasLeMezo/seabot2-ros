...
void DepthPoseNode::init_parameters() {
    ...
    this->declare_parameter<double>("velocity_limit", velocity_limit_);
    ...
    velocity_limit_ = this->get_parameter_or("velocity_limit", velocity_limit_);
...
void DepthPoseNode::pressure_callback(const pressure_ms5803_driver::msg::PressureSensorData &msg) {
    ...
    if(pressure_deque_.size()==filter_window_size_){
        ...
        if(depth_memory_.size()==(velocity_dt_gap_sample_+filter_velocity_window_size_)){
            ...
            /// Remove side values
            vector<double> velocity_median(velocities.begin()+filter_velocity_median_remove_side_samples_, velocities.end()-filter_velocity_median_remove_side_samples_);
            /// Sum elements and compute mean
            double velocity = std::accumulate(velocity_median.begin(), velocity_median.end(), 0.0)/velocity_median.size();
